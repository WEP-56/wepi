...
export type ConfigSettingsGroup = "models" | "extensions" | "other";
...
export function ConfigSettingsPanel(props: {
	...
	return (
		<ConfigModalErrorBoundary open={true} onClose={() => undefined}>
			<ConfigModalContent
				open={true}
				api={props.api}
				onClose={() => undefined}
				onSaved={props.onSaved}
				embeddedGroup={props.group}
			/>
		</ConfigModalErrorBoundary>
	);
}

function ConfigModalContent(props: ConfigModalProps & { embeddedGroup?: ConfigSettingsGroup }) {
	const { open, api, onClose, onSaved, embeddedGroup } = props;
	const [section, setSection] = useState<"config" | "skills" | "extensions" | "editors" | "logs">("config");
	...
	const handleExport = async () => {
		try {
			...
			a.href = url;
			// 文件名含时间戳，便于用户区分多次备份
			a.download = `wepi-config-${new Date().toISOString().slice(0, 10)}.json`;
			a.click();
	...
	useEffect(() => {
		if (!embeddedGroup) return;
		if (embeddedGroup === "models") {
			setSection("config");
			setTab("models");
		} else if (embeddedGroup === "extensions") {
	...
	const content = (
		<div className={embeddedGroup ? "config-modal embedded-config-panel" : "config-modal"}>
			{!embeddedGroup && (
				<div className="modal-header">
			...
			{embeddedGroup && section === "config" && (
				<div className="embedded-config-actions">
					<button className="config-btn primary" onClick={handleExport}>
						{t("common.export")}
					</button>
					<button className="config-btn blue" onClick={handleImport}>
				...
				<div className="config-layout">
					<aside className="config-sidebar" aria-label={embeddedGroup ? t("settings.title") : t("config.title")}>
						{(!embeddedGroup || embeddedGroup === "models") && (
						...
						{(!embeddedGroup || embeddedGroup === "extensions") && (
						<div className="config-sidebar-group">
						...
						{(!embeddedGroup || embeddedGroup === "other") && (
						<div className="config-sidebar-group">
	...
	if (embeddedGroup) return content;
